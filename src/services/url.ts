...
const url = {
    ...
    verifyEmail: (token?: string): IAppLinkHref => ({
        href: token ? "/account/verify-email?token=[token]" : "/account/verify-email",
        as: token ? `/account/verify-email?token=${token}` : "/account/verify-email"
    }),
